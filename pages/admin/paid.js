...
const paid = () => {
    ...
    const fetchBill = async () => {
        try {
            ...
            const response = await fetch(process.env.API_URL + '/bills/paid', {
                method: 'GET',
                headers: {
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {currentFilteredItems.map((mypaid, index) => (
                            <tr key={mypaid.room_id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">{mypaid.price + (mypaid.electricity_amount * mypaid.electricity_fee) + (mypaid.water_amount * mypaid.water_fee) + mypaid.maintenance_fee + mypaid.common_area_fee}</td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className={`${mypaid.bill_status === 1 ? 'bg-green-500' : 'bg-red-500'} p-2 rounded text-white`}>
                                        {mypaid.bill_status === 1 ? 'Paid' : 'Unpaid'}
                                    </span>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <button
                                            ...
                                            <div className="origin-top-right absolute mt-2 w-40 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
                                                <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                                                    <a
                                                        ...
                                                        role="menuitem"
                                                        onClick={() => openModal(mypaid.bill_id, mypaid.bill_status)}
                                                    >
                                                        Change Status
                                                    </a>
                                                    <a
                                                        className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900"
                                                        role="menuitem"
                                                        onClick={() => handleViewData(mypaid.bill_id)}
                                                    >
                                                        View Data
                                                    </a>
                                                    <a
                                                        className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900"
                                                        role="menuitem"
                                                        onClick={() => handleDelete(mypaid.bill_id)}
                                                    >
                                                        Delete
                                                    </a>
                                                </div>
