...
const edit_data_admin = () => {
    ...
    const fetchAdmin = async () => {
        try {
            ...
            const response = await fetch(`${process.env.API_URL}/users/admin/${id}`, {
                method: 'GET',
                headers: {
            ...
            if (result.status == 'success') {
                const updatedFormAdmin = {
                    ...result.message,
                    password: result.message.password_view,
                    repeatpassword: result.message.password_view
                };

                delete updatedFormAdmin.password_view;

                setFormAdmin(updatedFormAdmin);
            } else {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formAdmin.password !== formAdmin.repeatpassword) {
            Swal.fire({
                icon: 'error',
        ...
        try {
            ...
            const response = await fetch(`${process.env.API_URL}/users/edit_admin/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: 'Bearer ' + token,
    ...
    return (
        <>
            <div className="flex flex-col items-center mt-10 pb-20">
                <h1 className="text-3xl font-semibold mb-6">Edit Admin</h1>
                <form onSubmit={handleSubmit} className="w-80">
                    ...
                    <div className="mb-4">
                        ...
                        <input
                            type="text"
                            name="repeatpassword"
                            value={formAdmin.repeatpassword}
                            onChange={handleInputChange}
                            className="w-full px-4 py-2 border rounded focus:ring-blue-500 focus:border-blue-500"
                            required
                        />
